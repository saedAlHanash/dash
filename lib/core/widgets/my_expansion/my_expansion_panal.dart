...
class _MyExpansionPanelListState extends State<MyExpansionPanelList> {
  ...
  void _handlePressed(bool isExpanded, int index) {
    if (widget.children[index].onTapItem != null) {
      widget.children[index].onTapItem!(index);
    }
  ...
  Widget build(BuildContext context) {
    ...
    for (int index = 0; index < widget.children.length; index += 1) {
      if (_isChildExpanded(index) && index != 0 && !_isChildExpanded(index - 1)) {
        items
            .add(MaterialGap(key: _SaltedKey<BuildContext, int>(context, index * 2 - 1)));
      }
        ...
        MaterialSlice(
          ...
          child: widget.decoration == null
              ? MyCardWidget(
                  ...
                  elevation: widget.cardElevation ?? 5,
                  margin: EdgeInsets.all(widget.cardElevation ?? 5.0).r,
                  child: itemWidget,
                )
              : Container(
