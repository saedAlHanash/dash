...
class DeletePlanCubit extends Cubit<DeletePlanInitial> {
  DeletePlanCubit() : super(DeletePlanInitial.initial());

  Future<void> deletePlan(BuildContext context, {required int id}) async {
    final r = await NoteMessage.showConfirm(context, text: 'تأكيد العملية');
    if (!r) return;
    emit(state.copyWith(statuses: CubitStatuses.loading, id: id));
    final pair = await _deletePlanApi(id: id);
  ...
  Future<Pair<bool?, String?>> _deletePlanApi({required int id}) async {
    final response = await APIService()
        .callApi(type: ApiType.delete,url: DeleteUrl.deletePlan, query: {'Id': id});
