...
class DeleteCouponCubit extends Cubit<DeleteCouponInitial> {
  DeleteCouponCubit() : super(DeleteCouponInitial.initial());

  Future<void> deleteCoupon(BuildContext context, {required int id}) async {

    final r = await NoteMessage.showConfirm(context, text: 'تأكيد العملية');
    if (!r) return;

    emit(state.copyWith(statuses: CubitStatuses.loading, id: id));
    final pair = await _deleteCouponApi(id: id);
  ...
  Future<Pair<bool?, String?>> _deleteCouponApi({required int id}) async {
    final response = await APIService()
        .callApi(type: ApiType.delete,url: DeleteUrl.deleteCancelCoupon, query: {'Id': id});
