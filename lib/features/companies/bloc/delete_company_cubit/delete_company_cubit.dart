...
class DeleteCompanyCubit extends Cubit<DeleteCompanyInitial> {
  DeleteCompanyCubit() : super(DeleteCompanyInitial.initial());

  Future<void> deleteCompany(BuildContext context, {required int id}) async {
    final r = await NoteMessage.showConfirm(context, text: 'تأكيد العملية');
    if (!r) return;
    emit(state.copyWith(statuses: CubitStatuses.loading, id: id));
    final pair = await _deleteCompanyApi(id: id);
  ...
  Future<Pair<bool?, String?>> _deleteCompanyApi({required int id}) async {
    final response = await APIService()
        .callApi(type: ApiType.delete,url: DeleteUrl.deleteCompany, query: {'Id': id});
