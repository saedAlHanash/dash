...
class ProvidersPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocListener<ChangeProviderStateCubit, ChangeProviderStateInitial>(
      listenWhen: (p, c) => c.statuses.done,
      listener: (context, state) {
        context.read<ProvidersCubit>().getProviders();
      },
      child: Scaffold(
        body: BlocBuilder<ProvidersCubit, ProvidersInitial>(
          builder: (context, state) {
            ...
            if (list.isEmpty)
              return const NotFoundWidget(text: 'لا يوجد مزودات دفع');
            return Column(
              children: [
                DrawableText(
                  text: 'المزودات ',
