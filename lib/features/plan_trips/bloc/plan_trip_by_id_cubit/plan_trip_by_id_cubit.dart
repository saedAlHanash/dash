...
class PlanTripBuIdCubit extends Cubit<PlanTripBuIdInitial> {
  ...
  Future<Pair<PlanTrip?, String?>> _getPlanTripBuIdApi({required int id}) async {
    final response =
        await APIService().callApi(type: ApiType.get,url: GetUrl.planTripById, query: {'Id': id});

    if (response.statusCode == 200) {
      return Pair(PlanTrip.fromJson(response.json['result'] ?? {}), null);
    } else {
